...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (const find of restElements) {
    if (find < min) {
      min = find;
    } else if (find > max) {
      max = find;
...
function concatenateStrings(array, subString) {
  ...
  console.log(array.substring(array.lenght) ? true : false);
}
...
function replaceNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > 10) {
      arr[i] = "Greater than 10";
      rest = [...rest, arr[i]];
    } else if (arr[i] <= 10) {
      arr[i] = "Less than or equal to 10";
      rest = [...rest, arr[i]];
    }
  }
  return rest;
